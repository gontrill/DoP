...
::gpk::error_t									run										(::gme::SServer& server)		{
	...
	info_printf("Server running on %u.%u.%u.%u:%u."	, (uint32_t)local.IP[0]
													, (uint32_t)local.IP[1]
													, (uint32_t)local.IP[2]
													, (uint32_t)local.IP[3]
													, (uint32_t)local.Port
	...
	while (server.Listening) {		// Loop and get data from clients */
		...
		bi_if(1 == ::handleRequest(server, command.Command, local, sa_client), "Server received a close message.");
	}
	server.Listening								= false;
	sdsafe.Handle									= INVALID_SOCKET;
	closesocket(sd);
	server.Running									= false;
	info_printf("Server on %u.%u.%u.%u:%u terminating."	, (uint32_t)local.IP[0]
														, (uint32_t)local.IP[1]
														, (uint32_t)local.IP[2]
														, (uint32_t)local.IP[3]
														, (uint32_t)local.Port
